...
class BasePETCount(nn.Module):
    ...
    def __init__(self, backbone, num_classes, quadtree_layer='sparse', args=None, **kwargs):
        super().__init__()
        # self.backbone = backbone  # 사용되지 않음
        self.transformer = kwargs['transformer']  # decoder
    ...
    def points_queries_embed(self, samples, stride=8, src=None, **kwargs):
        """
        Generate point query embedding during training
        """
        # dense position encoding at every pixel location
        # samples: [8, 3, 256, 256]
        dense_input_embed = kwargs['dense_input_embed']  # dense_input_embed = self.pos_embed(samples), [8, 256, 256, 256]
        bs, c = dense_input_embed.shape[:2]

        # get image shape
        input = samples.tensors
        image_shape = torch.tensor(input.shape[2:])  # [256, 256]
        shape = (image_shape + stride//2 -1) // stride  # [259, 259] // 8 -> [32, 32]
        ...
        points_queries = torch.vstack([shift_y.flatten(), shift_x.flatten()]).permute(1,0) # 2xN --> Nx2, N: 32x32(1024)
        h, w = shift_x.shape

        # get point queries embedding
        query_embed = dense_input_embed[:, :, points_queries[:, 0], points_queries[:, 1]]  # [8, 256, 1024]
        bs, c = query_embed.shape[:2]  # [8, 256]
        # 원본 이미지 256x256에 대한 쿼리 포인트 32x32
        query_embed = query_embed.view(bs, c, h, w)  # [8, 256, 32, 32]

        # get point queries features, equivalent to nearest interpolation
        shift_y_down, shift_x_down = points_queries[:, 0] // stride, points_queries[:, 1] // stride  # 왼쪽 위 포인트
    ...
    def points_queries_embed_inference(self, samples, stride=8, src=None, **kwargs):
        """
        Generate point query embedding during inference
        """
        # dense position encoding at every pixel location
        dense_input_embed = kwargs['dense_input_embed']  # dense_input_embed = self.pos_embed(samples), [8, 3, 256, 256]
        bs, c = dense_input_embed.shape[:2]
    ...
    def get_point_query(self, samples, features, **kwargs):
        ...
        if 'train' in kwargs:
            query_embed, points_queries, query_feats = self.points_queries_embed(samples, self.pq_stride, src, **kwargs)
            query_embed = query_embed.flatten(2).permute(2, 0, 1) # NxCxHxW --> NxCx(HW) --> (HW)xNxC
            v_idx = None
        else:
            query_embed, points_queries, query_feats, v_idx = self.points_queries_embed_inference(samples, self.pq_stride, src, **kwargs)
    ...
    def predict(self, samples, points_queries, hs, **kwargs):
        ...
        Crowd prediction
        """
        outputs_class = self.class_embed(hs)  # [2, 8, 1024, 2] or [2, 8, 4096, 2]
        # normalize to 0~1
        outputs_offsets = (self.coord_embed(hs).sigmoid() - 0.5) * 2.0  # [2, 8, 1024, 2] or [2, 8, 4096, 2]
...
class PET(nn.Module):
    ...
    def __init__(self, backbone, num_classes, args=None):
        ...
        # csh: 논문에서 언급한 것처럼 sparse에서 dense로 넘어갈 때 포인트가 4개로 분화한다.
    ...
    def forward(self, samples: NestedTensor, **kwargs):
        ...
        # forward
        if 'train' in kwargs:
            out = self.train_forward(samples, features, **kwargs)
        else:
            out = self.test_forward(samples, features, **kwargs)   
        return out

    def pet_forward(self, samples, features, **kwargs):
        # context encoding
        src, mask = features[self.encode_feats].decompose()  # NestedTensor
        assert mask is not None  # csh: 근데 인코더에 들어갈 때도 마스크를 왜 넣지?
        ...
        split_map = self.quadtree_splitter(src)  # [8, 1, 4, 2]
        ...
        if 'train' in kwargs or (split_map_sparse > 0.5).sum() > 0:
            ...
            kwargs['dec_win_size'] = [16, 8]  # 8개 윈도우
            outputs_sparse = self.quadtree_sparse(samples, features, **kwargs)  # samples: 원본 이미지(HxW), features: FPN(64x64, 32x32), context_info: 인코더 출력(32x32) 
        else:
            outputs_sparse = None
        ...
        if 'train' in kwargs or (split_map_dense > 0.5).sum() > 0:
            ...
            kwargs['dec_win_size'] = [8, 4]  # 128개 윈도우
            outputs_dense = self.quadtree_dense(samples, features, **kwargs)  # samples: 원본 이미지(HxW), features: FPN(64x64, 32x32), context_info: 인코더 출력(32x32) 
        else:
            outputs_dense = None
    ...
    def train_forward(self, samples, features, **kwargs):
        outputs = self.pet_forward(samples, features, **kwargs)
    ...
    def test_forward(self, samples, features, **kwargs):
        outputs = self.pet_forward(samples, features, **kwargs)
